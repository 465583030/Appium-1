...
class Space(Login):
    def __init__(self):
        ...
        # Space - 企业空间创建
        self.Space_org_create = self.excel.cell(sheet_name='SPACE', rowno=2, colno=4)

        # Space - 文件夹中上传图片（个人）
        self.Space_folder_img_upload_id = self.excel.cell(sheet_name='SPACE', rowno=3, colno=4)

        # Space - 文件夹中列举图片（个人）
        self.Space_folder_list_img = self.excel.cell(sheet_name='SPACE', rowno=4, colno=4)

        # Space - 空间中删除联系人(删除客户)
        self.Space_delete_contact_id = self.excel.cell(sheet_name='SPACE', rowno=5, colno=4)

        # Space - 空间中添加联系人(新增客户)
        self.Space_add_contac_id = self.excel.cell(sheet_name='SPACE', rowno=6, colno=4)

        # Space - 空间公开设置
        self.Space_switch_setting = self.excel.cell(sheet_name='SPACE', rowno=7, colno=4)

        # Space - 文件夹列表(联系人所有）
        self.Space_folder_list_id = self.excel.cell(sheet_name='SPACE', rowno=8, colno=4)

        # Space - 文件夹浏览记录
        self.Space_folder_browse = self.excel.cell(sheet_name='SPACE', rowno=9, colno=4)

        # Space - 空间类型列表(私人空间)
        self.Space_type_list = self.excel.cell(sheet_name='SPACE', rowno=10, colno=4)

        # Space - 热门空间名
        self.Space_popular_name = self.excel.cell(sheet_name='SPACE', rowno=11, colno=4)

        # Space - 空间创建
        self.Space_private_create = self.excel.cell(sheet_name='SPACE', rowno=12, colno=4)

        # Space - 空间列表
        self.Spaces_list = self.excel.cell(sheet_name='SPACE', rowno=13, colno=4)

        # Space - 空间更新
        self.Space_update_id = self.excel.cell(sheet_name='SPACE', rowno=14, colno=4)

        # Space - 文件夹创建
        self.Space_folder_create = self.excel.cell(sheet_name='SPACE', rowno=15, colno=4)

        # Space - 文件夹更新
        self.Space_folder_update_id = self.excel.cell(sheet_name='SPACE', rowno=16, colno=4)

        # Space - 文件夹列表
        self.Space_folder_list = self.excel.cell(sheet_name='SPACE', rowno=17, colno=4)

        # Space - 空间概况
        self.Space_overview_id = self.excel.cell(sheet_name='SPACE', rowno=18, colno=4)

        # Space - 文件夹删除
        self.Space_folder_delete_id = self.excel.cell(sheet_name='SPACE', rowno=19, colno=4)

        # Space - 空间删除
        self.Space_private_delete_id = self.excel.cell(sheet_name='SPACE', rowno=20, colno=4)
    ...
    # Space - 空间列表
    def Space_list_api(self, data):
        # 1.组装token
        data = eval(data.replace("self.token", self.token))

        # 2. 发送请求
        response = self.R.get_function(self.Spaces_list, data)

        # 3.打印日志
        self.plog.printlog(data, response, describle='Space - 空间列表', url=self.Spaces_list, method='get')
    ...
    # Space - 私人空间类型列表
    def Space_private_type_list_api(self, data):
        # 1.组装数据
        data = data.replace("self.token", self.token)
